package com.alexdoub.gdaxmanager.api;
...
import com.alexdoub.gdaxmanager.api.service.GDAXService;
...
import com.alexdoub.gdaxmanager.models.OrderModel;
...
//    static String BASE_URL = "https://api-public.sandbox.gdax.com";
    ...
    private APIClient() {
        ...
        _authKey = GDAXManagerApp.getInstance().getString(R.string.key);
        _authPassphrase = GDAXManagerApp.getInstance().getString(R.string.passphrase);
        _authSecret = GDAXManagerApp.getInstance().getString(R.string.secret);

        if (_authKey.length() == 0 || _authPassphrase.length() == 0 || _authSecret.length() == 0) {
            ViewUtils.ShowToast(GDAXManagerApp.getInstance(), "WARNING - You don't have your auth keys set up. You cannot make authenticated calls!", false);
        }
                ...
                .addNetworkInterceptor(new Interceptor() {
                    ...
                    public okhttp3.Response intercept(Chain chain) throws IOException {
                        ...
                        if (response.code() >= 401 && response.code() <= 403) {
                            ViewUtils.ShowToast(GDAXManagerApp.getInstance(), "A network call failed authentication. Are your API keys properly set?");
                        }
                        return response;
                    }
                })
...
//        String key = GDAXManagerApp.getInstance().GetString(R.string.key);
//    }
